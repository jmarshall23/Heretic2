...
static unsigned int LM_AllocBlock(int w, int h, int *x, int *y)
{
	int		i, j;
	int		best, best2;
	unsigned int	texnum;
	...
	for (texnum = 0; texnum < MAX_LIGHTMAPS; texnum++)
	{
		...
		if (texnum > gl_lms.current_lightmap_texture)
			gl_lms.current_lightmap_texture = texnum;
...
static void LM_UploadBlock( void )
{
	for (int i = 0; i < gl_lms.current_lightmap_texture; i++)
	{
		...
		char name[512];
		void *targa_rgba;
		int targa_width, targa_height;

		GL_Bind(gl_state.lightmap_textures + i);
		qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		qglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

		snprintf(name, sizeof(name), "maps/ssdocks/LM_bsp%d_MeshShape.tga", i);

		//void LoadTGA(char *name, byte **pic, int *width, int *height);
		LoadTGA(name, &targa_rgba, &targa_width, &targa_height);
		//	LoadPCX (f);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, targa_width, targa_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, targa_rgba);
	}
}
