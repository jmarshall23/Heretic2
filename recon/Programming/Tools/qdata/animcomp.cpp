...
extern "C" void	*SafeMalloc(size_t n, char *desc);
...
extern "C" void AnimCompressInit(int nframes,int nVerts,int CompressedFrameSize)
{
	nFrames=nframes;
	MatWidth=nVerts*3;
...
extern "C" void AnimSetFrame(int frame,int index,float x,float y,float z)
{
...
extern "C" void AnimCompressDoit()
{
	float compression;
	float *rescale;
	float *ans;
...
extern "C" void AnimCompressToBytes(float *trans,float *scale,char *mat,char *ccomp,unsigned char *cbase,float *cscale,float *coffset,float *bmin,float *bmax)
{
	int k,l,nv,j;
	float maxdev;
	float avedev;
...
extern "C" void AnimCompressGetMatrix(float *mat)
{
	int k,l;
	for (k=0;k<MatWidth;k++)
		for (l=0;l<MatHeight;l++)
...
extern "C" void AnimCompressGetFrames(float *mat)
{
	memcpy(mat,bestcomp,CFrameSize*nFrames*sizeof(float));
}

extern "C" void AnimCompressGetBase(int i,float *x,float *y,float *z)
{
	*x=base[i*3];
	*y=base[i*3+1];
	*z=base[i*3+2];
