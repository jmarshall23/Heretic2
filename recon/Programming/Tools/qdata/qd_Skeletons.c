...
//    	mesh_node_t *nodesList;
	...
	while(TK_Search(TK_CLUSTER_NAME) != TK_EOF)
	{
		...
		{
			if(_stricmp(stripped, skeletonJointNames[skeletonNameOffsets[skelType]+i]) == 0)
			{
...
//	nodesList = SafeMalloc(MAX_FM_MESH_NODES * sizeof(mesh_node_t), "Mesh Node List");
	pmnodes = SafeMalloc(MAX_FM_MESH_NODES * sizeof(mesh_node_t), "Mesh Node List");
...
void ReadHRCClusterList(mesh_node_t *meshNode, int baseIndex)
{
	...
	while (nextToken == TK_CLUSTER_NAME)
	{
		...
		{
			if(_stricmp(stripped, skeletonJointNames[skeletonNameOffsets[g_skelModel.type]+i]) == 0)
			{
				...
				if(!baseIndex)
				{
					meshNode->clusters[i] = SafeMalloc(numVerts*sizeof(int), "ReadHRCClusterList");
				...
				else
				{
					...
					meshNode->clusters[i] = SafeMalloc((meshNode->num_verts[i+1]+numVerts)*sizeof(int), "ReadHRCClusterList");
					assert(meshNode->clusters[i]);
...
#define MAX_STACK 64
	...
	while(TK_Search(TK_NAME) != TK_EOF)
	{
		...
		if(_stricmp(stripped, skeletonRootNames[skeletonRNameOffsets[skelType]]) == 0)
		{
			break;
		}
	}
...
//		TK_Beyond(TK_MODEL);
		...
		while(TK_Search(TK_NAME) != TK_EOF)
		{
			...
			{	// labels indicated by the QDT
				if(_stricmp(stripped, RefPointNameList[k]) == 0)
				{
					break;
				}
			}
