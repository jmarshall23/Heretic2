...
*/
#define BIG_EPSILON 50	//unormalized dists have big values
...
float ComputeBoundingSphere(winding_t *source,vec3_t c)
{
	int i;
	float r,tr;
	vec3_t dif;
...
int BoundSphereReject(winding_t *source, winding_t *pass, winding_t *target)
{
	vec3_t cs,cp,ct,cone;
	float rs,rp,rt,h,rtot;
...
void SimpleFlood (portal_t *srcportal, int leafnum)
{
	...
	for (i=0 ; i<leaf->numportals ; i++)
	{
		...
		if (distcull)
		{
			VectorSubtract(srcportal->origin,p->origin,c);
			if (VectorLength(c)-srcportal->radius-p->radius>distcull)
				continue;
		}
...
void BasePortalVis (int portalnum)
{
	...
	if (p->portalfront == NULL)
		Error("BasePortalVis (front) MALLOC failed!  Could not allocate %s bytes.", portalbytes);
	memset (p->portalfront, 0, portalbytes);

	p->portalflood = malloc (portalbytes);
	if (p->portalflood == NULL)
		Error("BasePortalVis (flood) MALLOC failed!  Could not allocate %s bytes.", portalbytes);
	memset (p->portalflood, 0, portalbytes);
	
	p->portalvis = malloc (portalbytes);
	if (p->portalvis == NULL)
		Error("BasePortalVis (vis) MALLOC failed!  Could not allocate %s bytes.", portalbytes);
