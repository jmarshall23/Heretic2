...
winding_t *NewWinding (int points)
{
	...
	w = malloc (size);
	if (w == NULL)
		Error("NewWinding MALLOC failed!  Could not allocate %s bytes.", size);
	memset (w, 0, size);
...
//
	...
	{
		Error ("Vismap expansion overflow in ClusterMerge; vismap_p %n leafnum %i", vismap_p, leafnum);
	}
...
// each file portal is split into two memory portals
	...
	if (portals == NULL)
		Error("LoadPortals (portals) MALLOC failed!  Could not allocate %s bytes.", 2*numportals*sizeof(portal_t));
	memset (portals, 0, 2*numportals*sizeof(portal_t));
	
	leafs = malloc(portalclusters*sizeof(leaf_t));
	if (leafs == NULL)
		Error("LoadPortals (leafs) MALLOC failed!  Could not allocate %s bytes.", portalclusters*sizeof(leaf_t));
	memset (leafs, 0, portalclusters*sizeof(leaf_t));

	originalvismapsize = portalclusters*leafbytes;
	uncompressedvis = malloc(originalvismapsize);
	if (uncompressedvis == NULL)
		Error("LoadPortals (vis) MALLOC failed!  Could not allocate %s bytes.", originalvismapsize);
...
void CalcPHS (void)
{
	...
	//
		...
		if (vismap_p > vismap_end)
		{
			Error ("Vismap expansion overflow in CalcPHS; vismap_p %n count %i", vismap_p, count);
		}
...
int main (int argc, char **argv)
{
	...
	for (i=1 ; i<argc ; i++)
	{
		...
		else if (!strcmp(argv[i], "-distcull"))
		{
			distcull = atof(argv[i+1]);
			printf ("distcull = %f\n", distcull);
			i++;
		}
	...
	if (i != argc - 1)
		Error ("usage: vis [-threads #] [-distcull x] [-fast] [-v] bspfile");
