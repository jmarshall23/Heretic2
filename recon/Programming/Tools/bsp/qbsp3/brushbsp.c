...
tree_t *AllocTree (void)
{
	...
	if (tree == NULL)
		Error("AllocTree MALLOC failed!  Could not allocate %s bytes.", sizeof(*tree));
...
node_t *AllocNode (void)
{
	...
	if (node == NULL)
		Error("AllocNode MALLOC failed!  Could not allocate %s bytes.", sizeof(*node));
...
bspbrush_t *AllocBrush (int numsides)
{
	...
	bb = malloc(c);
	if (bb == NULL)
		Error("AllocBrush MALLOC failed!  Could not allocate %s bytes.", sizeof(c));
