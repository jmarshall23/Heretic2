...
void MakeTransfers (int i)
{
	...
	if (patch->numtransfers)
	{
		...
		if (patch->transfers == NULL)
			Error("MakeTransfers MALLOC failed!  Could not allocate %s bytes.", s);
...
void BounceLight (void)
{
	...
	printf ("bouncing light on %i patches\n", num_patches);
	for (i=0 ; i<num_patches ; i++)
	{
		p = &patches[i];
		for (j=0 ; j<3 ; j++)
...
#if 0
		if(radiosity[i][0] || radiosity[i][1] || radiosity[i][2])
		{
			int b;
...
int main (int argc, char **argv)
{
	...
	if (i != argc - 1)
		Error ("usage: qrad [-v] [-chop num(64)] [-scale num(1)] [-direct num(1)] [-entity num(1)] [-ambient num(0)] [-maxlight num(1.5)] [-threads num] [-bounce num(8)] [-extra] [-dump] bspfile");
