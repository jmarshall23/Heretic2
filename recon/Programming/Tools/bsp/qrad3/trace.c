...
void MakeTnodes (dmodel_t *bm)
{
	...
	malloctnodes = tnodes = malloc( (numnodes+1) * sizeof(tnode_t));
	if (tnodes == NULL)
		Error("MakeTnodes MALLOC failed!  Could not allocate %s bytes.", (numnodes+1)*sizeof(tnode_t));
...
void FreeTnodes ()
{
	if (malloctnodes == NULL)
		Error("FreeTnodes after free.\n");
	free(malloctnodes);
	malloctnodes = tnodes = NULL;
