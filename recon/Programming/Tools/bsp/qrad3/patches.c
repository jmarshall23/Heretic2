...
//		VectorScale (texture_reflectivity[i], 0.001, texture_reflectivity[i]);
...
#if 0
texture_reflectivity[i][0] = 0.1;
texture_reflectivity[i][1] = 0.1;
texture_reflectivity[i][2] = 0.1;
#endif
	}
}
...
void MakePatchForFace (int fn, winding_t *w)
{
	...
	if (fn < dmodels[0].numfaces)
	{
		...
		for (i=0 ; i<3 ; i++)
		{
			patch->baselight[i] *= patch->reflectivity[i];
		}
#endif
