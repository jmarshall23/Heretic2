...
triangulation_t	*AllocTriangulation (dplane_t *plane)
{
	...
	if (t == NULL)
		Error("AllocTriangulation MALLOC failed!  Could not allocate %s bytes.", sizeof(triangulation_t));
...
void CreateDirectLights (void)
{
	...
	for (i=0, p=patches ; i<num_patches ; i++, p++)
	{
		...
		if (dl == NULL)
			Error("CreateDirectLights (surfaces) MALLOC failed!  Could not allocate %s bytes.", sizeof(directlight_t));
	...
	for (i=0 ; i<num_entities ; i++)
	{
		...
		dl = malloc(sizeof(directlight_t));
		if (dl == NULL)
			Error("AllocDirectLights (entities) MALLOC failed!  Could not allocate %s bytes.", sizeof(directlight_t));
	...
	for (i = 0 ; i<dvis->numclusters ; i++)
	{
		...
		for (l=directlights[i] ; l ; l=l->next)
		{
			...
			if (!styletable[l->style])
			{
				...
				if (styletable[l->style] == NULL)
					Error("GatherSampleLight MALLOC failed!  Could not allocate %s bytes.", mapsize);
...
void BuildFacelights (int facenum)
{
	...
	if((texinfo[f->texinfo].flags & SURF_TALL_WALL) && (f->lighting.r|f->lighting.g|f->lighting.b))
	{
		return;
	}
	...
	if (styletable[0] == NULL)
		Error("BuildFaceLights (styletable) MALLOC failed!  Could not allocate %s bytes.", tablesize);
	...
	if (fl->origins == NULL)
		Error("BuildFaceLights (origins) MALLOC failed!  Could not allocate %s bytes.", sizeof(tablesize));
...
//			printf ("patch with no samples\n");
	...
	if(texinfo[f->texinfo].flags & SURF_TALL_WALL)
	{
		...
		for (patch = face_patches[facenum] ; patch ; patch=patch->next)
		{
			if (patch->samples)
			{
				f->lighting.r += patch->samplelight[0]/numPatches;
				f->lighting.g += patch->samplelight[1]/numPatches;
				f->lighting.b += patch->samplelight[2]/numPatches;				
			}
			else
			{
	//			printf ("patch with no samples\n");
...
void FinalLightFace (int facenum)
{
	...
	if ( texinfo[f->texinfo].flags & (SURF_WARP|SURF_SKY|SURF_TALL_WALL) )
		return;		// non-lit texture
