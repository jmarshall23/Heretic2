...
qboolean DoColor(int iIndex)
{
	...
	cc.rgbResult = GetEntityColor(iIndex);
...
//
		case ID_EDIT_UNDO:
			...
			break;
		case ID_EDIT_REDO:
			MessageBox (hWnd, "Trust me, you don't want this option", "NO", MB_OK);
			break;
...
//
		...
		case ID_VIEW_SHOWENT:
			...
			else
			{
				...
				if ( g_qeglobals.d_savedinfo.exclude & BUOY_ONLY)
				{
					...
					CheckMenuItem( GetMenu(g_qeglobals.d_hwndMain), ID_VIEW_SHOWBUOYONLY, MF_BYCOMMAND | MF_UNCHECKED);
				}
			}				
			Sys_UpdateWindows (W_XY|W_CAMERA);
			break;
		...
		case ID_VIEW_SHOWBUOYONLY:
			if ( ( g_qeglobals.d_savedinfo.exclude ^= BUOY_ONLY ) & BUOY_ONLY )
			{
				CheckMenuItem( GetMenu(g_qeglobals.d_hwndMain), ID_VIEW_SHOWBUOYONLY, MF_BYCOMMAND | MF_CHECKED);
				if (!( g_qeglobals.d_savedinfo.exclude & EXCLUDE_ENT))
				{
					g_qeglobals.d_savedinfo.exclude ^= EXCLUDE_ENT;
					CheckMenuItem( GetMenu(g_qeglobals.d_hwndMain), ID_VIEW_SHOWENT, MF_BYCOMMAND | MF_UNCHECKED);
				}
			}
			else
...
//				RunBsp (bsp_commands[LOWORD(wParam-CMD_BSPCOMMAND)]);
				bsp_StartBsp (bsp_commands[LOWORD(wParam-CMD_BSPCOMMAND)]);
			}
			break;
...
//
		...
		case ID_MISC_SELECTENTITYCOLOR:
			{
				...
				if ( ( inspector_mode == W_ENTITY ) && DoColor(COLOR_ENTITY) == true )
				{
					...
					SetWindowText( hwndEnt[EntKeyField], ENTKEY_COLOR );
					AddProp();
					SetWindowText( hwndEnt[EntValueField], buffer );
					SetWindowText( hwndEnt[EntKeyField], ENTKEY_COLOR_NOUNDERSCORE );
					AddProp();
				}
				Sys_UpdateWindows( W_ALL );
			}
		...
		case ID_MISC_SETMAXVIEWDISTANCE:
			DoSetMaxViewDist();
			break;
...
//
		case ID_BRUSH_3SIDED:
			UNDO_StartBrushEdit("&Undo 3side");
			Brush_MakeSided (3);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_3SNAP:
			UNDO_StartBrushEdit("&Undo 3side Snap");
			Brush_MakeSided_Snap (3);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_4SIDED:
			UNDO_StartBrushEdit("&Undo 4side");
			Brush_MakeSided (4);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_4SNAP:
			UNDO_StartBrushEdit("&Undo 4side Snap");
			Brush_MakeSided_Snap (4);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_5SIDED:
			UNDO_StartBrushEdit("&Undo 5side");
			Brush_MakeSided (5);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_5SNAP:
			UNDO_StartBrushEdit("&Undo 5side Snap");
			Brush_MakeSided_Snap (5);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_6SIDED:
			UNDO_StartBrushEdit("&Undo 6side");
			Brush_MakeSided (6);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_6SNAP:
			UNDO_StartBrushEdit("&Undo 6side Snap");
			Brush_MakeSided_Snap (6);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_7SIDED:
			UNDO_StartBrushEdit("&Undo 7side");
			Brush_MakeSided (7);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_7SNAP:
			UNDO_StartBrushEdit("&Undo 7side Snap");
			Brush_MakeSided_Snap (7);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_8SIDED:
			UNDO_StartBrushEdit("&Undo 8side");
			Brush_MakeSided (8);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_8SNAP:
			UNDO_StartBrushEdit("&Undo 8side Snap");
			Brush_MakeSided_Snap (8);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_9SIDED:
			UNDO_StartBrushEdit("&Undo 9side");
			Brush_MakeSided (9);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_9SNAP:
			UNDO_StartBrushEdit("&Undo 9side Snap");
			Brush_MakeSided_Snap (9);
			UNDO_FinishBrushEdit (NULL);
			break;
...
//
		case ID_BRUSH_FLIPX:
			UNDO_StartBrushEdit ("&Undo FlipX");
			Select_FlipAxis (0);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_FLIPY:
			UNDO_StartBrushEdit ("&Undo FlipY");
			Select_FlipAxis (1);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_FLIPZ:
			UNDO_StartBrushEdit ("&Undo FlipZ");
			Select_FlipAxis (2);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_ROTATEX:
			UNDO_StartBrushEdit ("&Undo RotateX");
			Select_RotateAxis (0, 90);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_ROTATEY:
			UNDO_StartBrushEdit ("&Undo RotateY");
			Select_RotateAxis (1, 90);
			UNDO_FinishBrushEdit (NULL);
			break;
		case ID_BRUSH_ROTATEZ:
			UNDO_StartBrushEdit ("&Undo RotateZ");
		...
		case ID_SELECTION_SCALE:
			DoScale();
			break;
		case ID_SELECTION_TOWER:
			DoTower();
			break;
...
// LIST TEXTURES IN USE

		case ID_TEXTURE_LISTINUSE:
			Texture_ListInUse();	
			break;

		case ID_ENTITY_LISTINUSE:
			Entity_ListInUse();
			break;
...
//	    QEW_SetupPixelFormat(maindc, false);
	...
	case WM_INITMENU:
		...
		case UNDO_NONE:
			hMenu = GetMenu(hWnd);
			ModifyMenu(hMenu, ID_EDIT_UNDO, MF_BYCOMMAND | MFT_STRING, ID_EDIT_UNDO,
				"Can't Undo");
			EnableMenuItem(hMenu, ID_EDIT_UNDO, MFS_DISABLED);
			break;
		case UNDO_BUSY:
			hMenu = GetMenu(hWnd);
			ModifyMenu(hMenu, ID_EDIT_UNDO, MF_BYCOMMAND | MFT_STRING, ID_EDIT_UNDO,
		...
		case UNDO_SUBTRACT:
			...
			ModifyMenu(hMenu, ID_EDIT_UNDO, MF_BYCOMMAND | MFT_STRING, ID_EDIT_UNDO,
				undobuffer.title);
			EnableMenuItem(hMenu, ID_EDIT_UNDO, MFS_ENABLED);
			break;
		default:
			hMenu = GetMenu(hWnd);
			ModifyMenu(hMenu, ID_EDIT_UNDO, MF_BYCOMMAND | MFT_STRING, ID_EDIT_UNDO,
		...
		// REDO will be enabled someday as well
		break;

	case WM_MOUSEWHEEL:// this Should be the message i get (and where i get it) in newest nt
		if (inspector_mode == W_ENTITY)
		{
			if ((short)HIWORD(wParam) > 0)
			{
				PostMessage (hwndEnt[EntList], WM_VSCROLL, SB_LINEUP, lParam);
			}
			else
			{
				PostMessage (hwndEnt[EntList], WM_VSCROLL, SB_LINEDOWN, lParam);
			}
		}
		else if (inspector_mode == W_TEXTURE) // i Believe this is ent window too, but it doesn't have to be, so stick with hwndTexture glob
		{
			PostMessage (g_qeglobals.d_hwndTexture, g_qeglobals.uMSH_MOUSEWHEEL, (short)HIWORD(wParam), lParam);
		}
		else //console uses entity window
		{
			if ((short)HIWORD(wParam) > 0)
			{
				PostMessage (g_qeglobals.d_hwndEdit, WM_VSCROLL, SB_LINEUP, lParam);
			}
			else
			{
				PostMessage (g_qeglobals.d_hwndEdit, WM_VSCROLL, SB_LINEDOWN, lParam);
			}
		}
		return 0;

	default://parent window gets mousewheel message in win95 & early nt--pass it on
		if (uMsg == g_qeglobals.uMSH_MOUSEWHEEL)
		{
			...
			{
				PostMessage (g_qeglobals.d_hwndTexture, g_qeglobals.uMSH_MOUSEWHEEL, (short)LOWORD(wParam), lParam);
			}
			else //console uses entity window
			{
				if (*(int *)&wParam > 0)
...
void Main_Create (HINSTANCE hInstance)
{
	...
	if ( ( hMenu = GetMenu( g_qeglobals.d_hwndMain ) ) != 0 )
	{
		...
		*/
		...
		if ( g_qeglobals.d_savedinfo.exclude & EXCLUDE_ENT )
			CheckMenuItem( hMenu, ID_VIEW_SHOWENT, MF_BYCOMMAND | MF_UNCHECKED );
		...
		if ( g_qeglobals.d_savedinfo.exclude & BUOY_ONLY )
		{
			if (g_qeglobals.d_savedinfo.exclude & EXCLUDE_LIGHTS )
			{
				CheckMenuItem( hMenu, ID_VIEW_SHOWBUOYONLY, MF_BYCOMMAND | MF_CHECKED );
			}
			else
			{
				g_qeglobals.d_savedinfo.exclude ^= BUOY_ONLY;
				CheckMenuItem( hMenu, ID_VIEW_SHOWBUOYONLY, MF_BYCOMMAND | MF_UNCHECKED );
			}
		}
	}
...
{ 
    ...
    tbb[6].iBitmap = 6; 
    tbb[6].idCommand = ID_SELECTION_SCALE; 
    ...
    tbb[7].iBitmap = 7; 
    tbb[7].idCommand = ID_SELECTION_SELECTCOMPLETETALL; 
    ...
    tbb[8].iBitmap = 8; 
    tbb[8].idCommand = ID_SELECTION_SELECTTOUCHING; 
    ...
    tbb[9].iBitmap = 9; 
    tbb[9].idCommand = ID_SELECTION_SELECTPARTIALTALL; 
    ...
    tbb[10].iBitmap = 10; 
    tbb[10].idCommand = ID_SELECTION_SELECTINSIDE; 
    ...
    tbb[12].iBitmap = 12;
    tbb[12].idCommand = ID_SELECTION_MAKEHOLLOW; 
    ...
    tbb[15].iBitmap = 15;
    tbb[15].idCommand = ID_VIEW_TRILINEAR; 
    ...
    tbb[19].iBitmap = 17;
    tbb[19].idCommand = ID_TEXTURE_LISTINUSE; 
    ...
    tbb[20].iBitmap = 18;
    tbb[20].idCommand = ID_ENTITY_LISTINUSE; 
