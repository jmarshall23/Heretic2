...
void Map_BuildBrushData(qboolean firsttime)
{
	brush_t	*b, *next;
	int			angle;
	...
	for (b=active_brushes.next ; b != NULL && b != &active_brushes ; b=next)
	{
		...
		if (strcmp(b->owner->eclass->name,"worldspawn") &&
			(b->rotatedonload != true) &&
			(b->owner->eclass->fixedsize) &&
			(firsttime == true)) //  entity
		{
			b->rotatedonload = true;
			angle = FloatForKey(b->owner, "angle");	//  this will read in entity facings correctly and draw
			if (angle > -1)
			{
				Select_Brush(b);
				Select_RotateAxis( 2, 0-angle );
				Select_Deselect();
			}
		}
		if (!b->brush_faces)
...
void Map_SaveFile (char *filename, qboolean use_region )
{
	...
	while (swapmade)
	{
		...
		{
	//		lame bubble sort by	e->origin[2], but hey, it fits this editor's style well
	//		**** set up to see if stuff exists		
			if (e->prev != &entities)
			{
				prev = e->prev;
			}
	...
	//		**** end of set up to see if stuff exists

			if (e->origin[2] > next->origin[2])
			{	//swap 'em
				swapmade = true;
				if (prev)
				{
