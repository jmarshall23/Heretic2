...
// get the size	
	text = COM_Parse (text);
	if (com_token[0] == '(')
	{	// parse the size as two vectors, mins and maxs
		e->PhysicsModel = false;
		e->fixedsize = true;
	...
	else if (!strcmp(com_token,"PHYSICS_MODEL"))
	{	// use the brushes
		e->PhysicsModel = true;	
	}
	else
...
// any remaining words are parm flags
	p = parms;
	for (i=0 ; i<LAST_LOW_SPAWNFLAG ; i++)
	{
		p = COM_Parse (p);
		if (!p)
			break;
	...
	for (i=FIRST_HIGH_SPAWNFLAG ; i<LAST_HIGH_SPAWNFLAG ; i++)
	{
		p = COM_Parse (p);
		if (!p)
			break;
		strcpy (e->flagnames[i - (FIRST_HIGH_SPAWNFLAG - LAST_LOW_SPAWNFLAG)], com_token);
	} 
