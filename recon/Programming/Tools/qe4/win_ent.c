...
BOOL UpdateSel(int iIndex, eclass_t *pec)
{
	...
	for (i=0 ; i<LAST_LOW_SPAWNFLAG ; i++)	//set the first 8 defined by code
	{
	...
	//need to set the last 8 as well

	for (i=FIRST_HIGH_SPAWNFLAG ; i<LAST_HIGH_SPAWNFLAG ; i++)	//set the last 9 defined by code
	{
		HWND hwnd = hwndEnt[EntCheck1+i];
		if (pec->flagnames[i - (FIRST_HIGH_SPAWNFLAG - LAST_LOW_SPAWNFLAG)] && pec->flagnames[i - (FIRST_HIGH_SPAWNFLAG - LAST_LOW_SPAWNFLAG)][0] != 0)
		{
			EnableWindow(hwnd, TRUE);
			SendMessage(hwnd, WM_SETTEXT, 0, (LPARAM)pec->flagnames[i - (FIRST_HIGH_SPAWNFLAG - LAST_LOW_SPAWNFLAG)]);
		} else {
...
void CreateEntity(void)
{
	...
	//sz is new entity name
	//pecNew is the entity data
...
void AddProp(void)
{
	...
	if (selected_brushes.next != selected_brushes.prev)
	{
		brush_t	*b;

		if (!strcmp(key,"angle") && (edit_entity->eclass->fixedsize)) // don't let them add angle to multiple FIXEDSIZED entities,
		{						  // even then, don't rotate them
			Sys_Printf ("Can't add angle property to multiple entities.\n");
			return;
		}
	...
	{
		oldangle = FloatForKey(edit_entity, "angle");
		SetKeyValue(edit_entity, key, value);
		if (!strcmp(key,"angle") && (edit_entity->eclass->fixedsize))
		{
			angle = FloatForKey(edit_entity, "angle");
			Select_RotateAxis( 2, oldangle-angle);
		}
	}
...
int GetEntityColor(int iIndex)
{
	...
	if ((iIndex == COLOR_ENTITY) && (KeyExists(edit_entity, ENTKEY_COLOR)))
	{
		GetVectorForKey (edit_entity, ENTKEY_COLOR, colorVec);
		rgbclr = 
			(int)(colorVec[0]*255) +
			(((int)(colorVec[1]*255))<<8) +
			(((int)(colorVec[2]*255))<<16);
	}
	else
	{
		rgbclr = 
...
void DelProp(void)
{
	...
	if (selected_brushes.next != selected_brushes.prev)
	{
		...
		if (!strcmp(sz,"angle")&& (edit_entity->eclass->fixedsize)) // don't let them delete angle from multiple FIXEDSIZED brushes
		{
			Sys_Printf("Can't delete angle from multiple entities.\n");
			return;
		}
		for (b=selected_brushes.next ; b != &selected_brushes ; b=b->next)
	...
	{
		if (!strcmp(sz,"angle") && (edit_entity->eclass->fixedsize))
		{
			angle = FloatForKey(edit_entity, "angle");
			if (angle > -1)
			{
				Select_RotateAxis(2, angle-0);
			}// return it to zero
		}
...
void SizeEntityDlg(int iWidth, int iHeight)
{
	...
	MOVE(g_qeglobals.d_hwndEdit,	DlgXBorder, DlgYBorder+TEXMODIFY_HEIGHT,iWidth - (2 * DlgXBorder), iHeight - (2 * DlgYBorder) );
	...
	MOVE(g_qeglobals.d_hwndTexture,	DlgXBorder, DlgYBorder+TEXMODIFY_HEIGHT, iWidth - (2 * DlgXBorder), iHeight - (2 * DlgYBorder) );
	...
	MOVE(hwndEnt[EntList], DlgXBorder, DlgYBorder+TEXMODIFY_HEIGHT, w, 2 * y);
	
	MOVE(hwndEnt[EntComment],
		DlgXBorder, 2 * DlgYBorder + 2 * y+TEXMODIFY_HEIGHT, 
		w, y - (2 * DlgYBorder));
	...
	for (iRow = 0; iRow <= NUM_SPAWNFLAGS; iRow += 7)
	{
		y = iHeight/2+TEXMODIFY_HEIGHT;
	...
	//
	yCheck = 20;
	y = iHeight/2 + 6 * yCheck; // + TEXMODIFY_HEIGHT;

	w = iWidth - (2 * DlgXBorder);
	h = (iHeight - (yCheck * 5 + 2 * DlgYBorder) ) - y;

	MOVE(hwndEnt[EntProps], DlgXBorder, y+TEXMODIFY_HEIGHT, w, h);

	y += h + DlgYBorder+TEXMODIFY_HEIGHT;
    ...
    LPARAM lParam)	// second message parameter
{
	...
	case WM_COMMAND: 
		...
		case IDC_E_0:
			if (selected_brushes.next->owner != NULL)
			{
				if ((edit_entity->eclass->fixedsize) && (selected_brushes.next != selected_brushes.prev))
					break;
				angle = FloatForKey(edit_entity, "angle");
				SetKeyValue (edit_entity, "angle", "0");
				SetFocus (g_qeglobals.d_hwndCamera);
				SetKeyValuePairs ();
				if ((angle > -1) && (edit_entity->eclass->fixedsize)
					&& (selected_brushes.next == selected_brushes.prev)) // only one FIXEDSIZED entity can be selected
				{
					Select_RotateAxis( 2, angle );
				}
			}
			break;
		case IDC_E_45:
		...
		case IDC_CHECK13:	//more spawn flags
		case IDC_CHECK14:
		case IDC_CHECK15:
		case IDC_CHECK16:
		case IDC_CHECK17:
