...
#ifdef _QDATA
	#define ARTDIRNAME "art"
	extern qboolean	g_publishOutput;
	char		g_publishDir[1024];
...
void SetQdirFromPath (char *path)
{
	...
	{
		if ((*(c-1)=='/' || *(c-1)=='\\')&&!Q_strncasecmp (c, BASEDIRNAME, len))
		{
	...
	for (c=path+strlen(path)-1 ; c != path ; c--)
	{
		...
		if ((*(c-1)=='/' || *(c-1)=='\\')&&!Q_strncasecmp (c, ARTDIRNAME, len))
		{
			qdirlen = c-path;
			strncpy (qdir, path, qdirlen);	// Copy the path before the ARTDIRNAME to qdir.
			...
			while (*c)
			{
				if (*c == '/' || *c == '\\')
				{
					...
					strncpy(c, BASEDIRNAME, strlen(BASEDIRNAME));
					c += strlen(BASEDIRNAME);
					strcpy(c, "/");
...
{
	char	*b;
	b = SafeMalloc(strlen(s)+1, "copystring");
	strcpy (b, s);
	return b;
}
...
int    LoadFile (char *filename, void **bufferptr)
{
	...
	buffer = SafeMalloc (length+1, "LoadFile");
...
int    TryLoadFile (char *filename, void **bufferptr)
{
	...
	*bufferptr = NULL;
	...
	if (!f)
		return -1;
	length = Q_filelength (f);
	buffer = SafeMalloc (length+1, "TryLoadFile");
	((char *)buffer)[length] = 0;
	SafeRead (f, buffer, length);
	fclose (f);
...
void	CreatePath (char *path)
{
	...
	if (g_nomkdir)
		return;		// Running a mkdir unecessarily on a server with no file creation rights can halt qdata.
